...
export const fetchDestinations = async (query: string): Promise<Destination[]> => {
  console.log(`fetchDestinations called with query: "${query}"`);
...
export const fetchDestinationById = async (id: number): Promise<Destination | null> => {
  console.log(`fetchDestinationById called with id: ${id}`);
  await simulateDelay(500); 
...
export const fetchNearbyDestinations = async (
  latitude: number,
  longitude: number
): Promise<Destination[]> => {
  console.log(`fetchNearbyDestinations called with latitude: ${latitude}, longitude: ${longitude}`);
  await simulateDelay(500);
  ...
  const sortedDestinations = destinations
    .map((destination) => ({
      ...destination,
      distance:
        Math.abs(destination.latitude - latitude) + Math.abs(destination.longitude - longitude),
    }))
